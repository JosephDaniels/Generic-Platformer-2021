...
class Event(object):
    ...
    def clone_attribs_from(self, pe):
        ...
        attribs = ["gain", "state", "unicode", "key", "mod", "pos", "rel",
                   "button", "size", "w", "h", "joy", "axis", "value"]
        for k in attribs:
            if hasattr(pe, k):
                setattr(self, k, getattr(pe, k))
...
class EventManager(object):
    ...
    Listener objects register themselves with the manager via
    ...
    Pygame events are automatically wrapped, an incomplete list of events
    you can listen for are:

        KeyDown, KeyUp, MouseMotion, MouseButtonUp, MouseButtonDown,
        JoyAxisMotion, JoyButtonUp, JoyButtonDown
    ...
    def run(self):
        ...
        while self.is_running:
            ...
            self.post_event("tick", time=new_time, dt =(new_time-prev_time))
...
class Widget(object):
    ...
    Widgets are rectangular elements where they have these values:
    ...
    (.box_x, .box_y)      .box_width
       ...
       the .pad_xxx memeber act as an invisble force field to push
       the widget away from the other elements. The .padding member will
       set all the sides simultaneously. Padding is used to space elements
       when container widgets .pack() their children. Padding is ignored if
       elements are positioned absolutely with the .x, .y members.
    ...
    padding = property(fset = _set_padding)
...
class TextEdit(Widget):
        ...
        if self.is_editable:
            ...
            if key == "backspace":
                ...
                if len(key)==1:
                    self.text += key
            self.redraw()
            self.announce("text_changed", text=self.text)
    ...
    def on_sendme(self, evt):
        msg_target = evt.target_address
        text = self.text
        self.ev_manager.post_event(msg_target, text = text)
    ...
    def pack(self):
        ...
        for  row, height in zip(self.rows, self.heights):
            ...
            for widget, width in zip(row, self.widths):
                if widget:
                    widget.x = x+widget.pad_left
                    widget.y = y+widget.pad_top
                x += width
            y += height
...
class Console(Widget):
    def __init__(self, parent=None, **kwargs):
        super(Console, self).__init__(parent, **kwargs)
        self.lines = []
        self.wordwrap = True
        font_name = pygame.font.get_default_font()
        self.font = pygame.font.SysFont(font_name, self.font_size)
